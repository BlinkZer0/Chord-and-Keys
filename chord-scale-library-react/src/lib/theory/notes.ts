...
export function toSharpName(n: string): string { 
  return ENHARMONIC_MAP[n] || n; 
}

export function pcIndex(note: string): number | null {
  const n = String(note).trim();
  const m = n.match(/^([A-G])([#b+-]*)/);
  if(!m) return null;
  
  const BASE: { [key: string]: number } = {C:0, D:2, E:4, F:5, G:7, A:9, B:11};
  let val = BASE[m[1]];
