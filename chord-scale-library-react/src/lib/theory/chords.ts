...
export function buildChord(root: string, quality: string): string[] { 
  ...
  const pattern = CHORD_QUALITIES[quality as keyof typeof CHORD_QUALITIES] || CHORD_QUALITIES.Maj; 
  if(rootPc == null) return []; 
  return pattern.map(iv => pcName(rootPc + iv)); 
}

export function chordToMidi(notes: string[], root: string, baseOct: number = 4): number[] {
...
export function eqArr(a: number[], b: number[]): boolean { 
...
export function invOf(midiNotes: number[], rootPc: number): string { 
