...
export function buildScale(tonic: string, modeName: string): string[] { 
  const rootPc = pcIndex(tonic); 
  const pattern = MODES[modeName as keyof typeof MODES] || MODES.Ionian; 
  if(rootPc == null) return []; 
  return pattern.map(iv => pcName(rootPc + iv)); 
}

export function scaleToMidi(scaleNotes: string[], tonic: string, startOct: number = 4): number[] {
